...
### 3.2 GET `/categories` — カテゴリ一覧

- **条件**: `activity_categories.is_deleted = false`
- **レスポンス**: `[{ "id", "name" }, ...]`

### 3.3 POST `/routines` — ルーティン定義登録
...
### 3.4 DELETE `/routines/{routine_id}` — 論理削除

- `UPDATE plan.routine SET is_deleted = true WHERE id = ? AND is_deleted = false`
- 更新 0 件: **404**
- レスポンス: メッセージ JSON

### 3.5 PUT `/routines/{routine_id}` — ルーティン定義編集

**ボディ**

- `title`, `activity_category_id`, `adapt`（登録と同じ）
- `adjust`: `adjust` が無い（または `null`）場合は調整を解除、ある場合は登録と同じ形で調整内容を更新

**処理概要**

1. 対象ルーティンを `is_deleted = false` で取得（存在しなければ 404）
2. `public.activity_categories` を検証（削除済みは無効。無効なら 400）
3. `plan.routine_adapt_day` を更新（`adapt_id` は変更しない）
4. `adjust` の有無に応じて `plan.routine_adjust_day`（既存更新 or 新規追加）または `plan.routine.adjust_id = NULL` を更新
5. `plan.routine.title` / `activity_category_id` を更新（`title` はユニーク）

**エラー**

- ルーティンが見つからない／削除済み: **404**
- `activity_category_id` が無効: **400**
- `title` ユニーク違反: **409**

**レスポンス**: `{ "id": 編集対象 routine id }`（200）

### 3.6 POST `/routines/{routine_id}/apply` — 個別反映
...
### 3.7 POST `/routines/apply-all` — 全件反映

**ボディ**: 同上 `year`, `month`

- `is_deleted = false` の全 `plan.routine` に対し、**3.6 と同じロジック**を順に適用
